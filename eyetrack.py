...
predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")#dlib를 사용했기때문에 결국은 눈동자를 따라가는것이 아닌 안와? 그 부분을 따라가는것임 일단은 데모버전 수정 필요

# Load the image to be placed on the eyes
image = cv2.imread("Rear View Mirror.jpg")
...
while True:
    ...
    cv2.imshow("Eye Tracking with Image", frame)
